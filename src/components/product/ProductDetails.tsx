...
const ProductDetails = ({ product }: { product: ProductType }) => {
  ...
  const handleCheckPincode = async () => {
    ...
    if (!res || !res.data?.success) {
      setError(res?.data?.message || "Failed to check pincode");
      setChecking(false);
      return;
    }
    const { best_courier } = res.data;
    setDeliveryInfo({
      charge: best_courier.rate,
      courier: best_courier.courier_name,
      deliveryDate: best_courier.etd ? best_courier.etd : "N/A",
    });
    setHasChecked(true);
    setChecking(false);  
  };
      ...
      <section className="w-full max-md:mt-4">
        <div className="flex flex-col lg:flex-row gap-4 md:gap-10">
          ...
          <div className="flex flex-col gap-3 w-full lg:w-[50%] xl:w-[55%] xxl:w-[60%]">
            ...
            >
              <span className="flex items-center gap-1 bg-define-brown text-white px-2 py-0.5 rounded text-sm">
                {product.averageRating.toFixed(1)}{" "}
                <FaStar size={14} fill="white" />
              </span>
            ...
            <div className=" flex items-center gap-2">
              ...
              {stockStatus === "available" && (
                <div className="flex items-center gap-1 bg-define-red/10 text-define-red px-5 py-2 rounded-full text-sm font-medium">
                  <div className="w-2 h-2 bg-define-red rounded-full" />
            ...
            <div className="flex items-center gap-2 w-full">
              <input
                ...
                maxLength={6}
                className="border w-full border-gray-200 outline-none placeholder:text-gray-400 text-define-brown rounded-md px-3 py-2 text-sm md:w-40"
              />
              <button
                type="button"
            ...
            {/* Offers */}
            <div>
              <h2 className="text-define-brown font-semibold">
                Available Offers
              </h2>
            ...
            {product.longDescription && (
              <div className="pt-6">
                ...
                <div
                  className="text-sm text-gray-700 leading-relaxed space-y-3 text-justify border-l-3  border-define-brown pl-2"
                  dangerouslySetInnerHTML={{
