...
const CartList = () => {
  const router = useRouter();
    const {customer, refreshCustomer, loading} = useCustomer();
...

  {
   ...
   const availableCartItems = (cart as CartType[]).filter((item) => {
    ...
    const totalMRP = availableCartItems.reduce(
    ...
    <div className="flex flex-col lg:flex-row justify-between items-center  h-[calc(100vh-100px)] overflow-y-auto no-scrollbar">
      ...
      <div className="flex flex-col gap-6 lg:px-2 px-0 self-start basis-full w-full">
        ...
        <div className="flex flex-col gap-4 w-full">
          {reversedCart.length === 0 ? (
            <p className="text-gray-500 text-center w-full">Your cart is empty.</p>
          ) : (
                ...
                >
                  ...
                  <div className="flex flex-col items-center w-full md:w-40">
                    <div className="relative w-full  h-[20rem] md:h-40  bg-gray-100 rounded overflow-hidden">
                      <Image
                        src={
                          (item.productId as ProductType).coverImage.url || ""
                        }
                    ...
                    <div className="md:flex hidden items-center mt-3">
                      <button
                        ...
                        className="size-[2rem] border  border-gray-200 rounded-full hover:bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed"
                        disabled={item.quantity <= 1 || isOutOfStock}
                      >
                        −
                      </button>

                      <span className=" h-[2rem] w-fit px-4 flex justify-center items-center border text-define-red  border-gray-200 mx-2 min-w-[40px] text-center">
                        {item.quantity}
                      </span>
                  ...
                  <div className="flex-1 flex flex-col justify-between">
                    <Link
                      href={`/product/${(item.productId as ProductType).slug}`}
                    >
                    ...
                    <div className="md:flex hidden gap-2 lg:gap-4 lg:justify-evenly items-center lg:justify-start md:pb-1 max-lg:mt-3">
                      ...
                      <button
                        ...
                        className={`font-semibold text-sm lg:text-base  ${
                          isOutOfStock ? "text-red-400" : " hover:text-red-500"
                        }`}
                      >
                        REMOVE
                      </button>
                    ...
                    <div className="flex md:hidden items-center gap-4 mt-3 w-full">
                      ...
                      <div className="flex lg:hidden items-center border border-gray-200 rounded-md overflow-hidden h-[40px]">
                        <button
                          onClick={() => handleQty(item, item.quantity - 1)}
                          className="px-2 h-full bg-gray-50  hover:bg-gray-200 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                          disabled={item.quantity <= 1 || isOutOfStock}
                        >
                          −
                        </button>

                        <span className="px-3 h-full flex items-center justify-center text-sm font-semibold  border-x border-gray-200 min-w-[40px]">
                          {item.quantity}
                        </span>
      ...
      ${
        isOutOfStock
          ...
          : "border-gray-300  hover:border-defined-green hover:text-defined-green hover:bg-green-50"
      }`}
                        onClick={() => handleMoveToWishlist(item)}
                        disabled={isOutOfStock}
                      >
      ...
      <div className="self-start w-full lg:w-[40%]">
        {reversedCart.length > 0 && (
            ...
            <button
              ...
              className={`mt-6 w-full rounded-full py-2 h-12 text-sm font-semibold transition-all ${
                availableCartItems.length === 0
                  ...
                  : "btn-grad  hover:scale-[1.03] text-white"
              }`}
            >
              {availableCartItems.length === 0
                ? "No Available Items"
