...
export default function Orders() {
  ...
  const getVariantText = (order: OrderType) => {
    ...
    return vars.map((v : { name: string; values: string[] }) => `${v.name}: ${v.values.join(", ")}`).join(" | ");
  };
  ...
  if (loading && page === 1) {
    return (
      <div className="bg-white p-8 rounded-lg text-center text-gray-500 shadow-sm w-full">
        Loading...
      </div>
    );
  }
  ...
  return (
    <div className="px-4 flex flex-col gap-4 h-[calc(100vh-100px)] overflow-y-auto no-scrollbar">
      {isFetching && (
        <div className="fixed inset-0 bg-white/50 z-50 flex items-center justify-center pointer-events-none">
          <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-define-brown"></div>
        </div>
      )}
      ...
      {orders?.map((order) => {
        ...
        return (
          ...
          >
            {/* HEADER */}
            <div className="grid grid-cols-2 lg:grid-cols-4 items-start lg:items-center justify-between gap-4 p-4 bg-define-brown text-white text-sm">
              <div>
              ...
              <div className="">
                <p className="text-white">Ship To</p>
            ...
            >
                    ...
                    <div className="flex md:flex-row flex-col gap-3  mt-4 w-full">
                      {isProductAvailable ? (
                        <button
                          ...
                          className={`lg:px-6 lg:py-4 p-3 text-sm  lg:text-sm bg-define-brown text-white rounded hover:bg-define-red w-full md:w-fit text-center ${
                            stockStatus === 0
                              ? "bg-gray-300 text-gray-500 cursor-not-allowed opacity-60"
                              : "bg-define-brown hover:bg-define-red text-white"
                          }`}
                        >
                          Buy Again
                        </button>
              ...
              <div className="flex flex-col gap-3 md:gap-2 w-full md:w-56">
                ...
                <button
                  ...
                  className="w-full md:px-4 md:py-2 p-3 text-sm bg-define-brown text-white rounded hover:bg-define-red"
                >
                  Track Your Order
                </button>
                ...
                {order.status === "Delivered" && (
                  <Link href={`/review?product=${order.product?.slug}`}>
                    <button className="w-full px-4 py-2 text-sm bg-define-brown text-white rounded hover:bg-define-red">
                      Write a Review
                    </button>
                  </Link>
                )}
            ...
            {getPageNumbers().map((pageNum, idx) => (
              <button
                ...
                className={`
                  ...
                  ${
                    pageNum === page
                      ? "bg-define-brown text-white shadow-md"
                      : pageNum === "..."
                        ? "text-gray-400 cursor-default"
                        : "bg-white border border-gray-200 text-gray-700 hover:bg-green-50 hover:text-define-red hover:border-green-200"
                  }
                `}
              >
                {pageNum}
